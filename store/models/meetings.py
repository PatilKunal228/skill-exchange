...
# Define India timezone (IST)
INDIA_TIMEZONE = pytz.timezone('Asia/Kolkata')

def current_india_date():
    """Get the current date in the India timezone."""
    return timezone.now().astimezone(INDIA_TIMEZONE).date()

def current_india_time():
    """Get the current time in the India timezone."""
    return timezone.now().astimezone(INDIA_TIMEZONE).time()
...
class Meeting(models.Model):
    ...
    meeting_link = models.URLField()  # Unique meeting link
    meeting_code = models.CharField(max_length=10, unique=True)  # Random meeting code
    date = models.DateField(default=current_india_date)  # Default to current date in India timezone
    time = models.TimeField(default=current_india_time)  # Default to current time in India timezone

    def __str__(self):
        return self.title

    def get_india_datetime(self):
        """Convert the stored date and time to India timezone."""
        # Combine date and time fields into a naive datetime object
        naive_datetime = timezone.datetime.combine(self.date, self.time)
        ...
        india_datetime = utc_datetime.astimezone(INDIA_TIMEZONE)
